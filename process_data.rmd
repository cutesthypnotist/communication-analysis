...
invisible(lapply(c("dplyr", "tidyr", "rstatix"), install_if_missing))

library(dplyr)
library(tidyr)
library(rstatix)

# Participant-level proportions of dominant strategy by gaze condition
strategy_props <- instructional_strategies_data_dt_merged_tt_tc %>%
  filter(
    task_type %in% c("Alignment", "Match"),
    !is.na(instructional_strategy),
    !is.na(gaze_condition)
  ) %>%
  mutate(
  ...
  count(subject_id, gaze_condition, instructional_strategy, name = "n_tasks") %>%
  complete(
    subject_id,
    gaze_condition,
    instructional_strategy,
  ...
  mutate(prop = n_tasks / sum(n_tasks)) %>%
  ungroup()

# Paired Wilcoxon tests by strategy
strategy_prop_tests <- strategy_props %>%
  group_by(instructional_strategy) %>%
  wilcox_test(prop ~ gaze_condition, paired = TRUE, detailed = TRUE) %>%
  adjust_pvalue(method = "holm") %>%
  add_significance("p.adj")

# Effect sizes
strategy_prop_effects <- strategy_props %>%
  group_by(instructional_strategy) %>%
  wilcox_effsize(prop ~ gaze_condition, paired = TRUE)

strategy_prop_tests
strategy_prop_effects
```
...
```{r my_big_plot, fig.width=6, fig.height=6, dpi=300}
library(dplyr)
library(ggplot2)
library(scales)
...
ggplot(pie_by_gaze, aes(x = 1, y = pct, fill = instructional_strategy)) +
  geom_col(width = 1.0, color = "white") +
  coord_polar(theta = "y") +
...
#Caption: Instructional Strategies Across All Subjects by Gaze Condition

#Double check counts
pie_by_gaze_count <- pie_by_gaze %>%
  group_by(gaze_condition) %>%
...
combined_segments_per_round_all_dt_merged
# First remove all task_type Locate
...
task_durations_cond <- combined_segments_per_round_all_dt_merged %>%
  group_by(subject_id,
           ...
           task_condition) %>%
  summarise(
    mean_task_duration = median(task_duration, na.rm = TRUE),  # median 🔢
    .groups = "drop"
  )

# 2. Factor‐level ordering for consistent legend
...
combined_nolocate <- combined_segments_per_round_all_dt_merged %>%
  filter(task_type != "Locate")  # filter 🚫

# Compute per‐subject medians for task_type
...
task_durations_cond2 <- combined_segments_per_round_all_dt_merged %>%
  group_by(
    ...
    instructional_strategy,
    task_condition,
    gaze_condition               # include this column 🔍 [dplyr group_by](https://www.google.com/search?q=dplyr+group_by)
  ) %>%
  summarise(
    mean_task_duration = median(task_duration, na.rm = TRUE),
    .groups = "drop"
...
# Change values in task_condition so that Correct = No Student Error and Incorrect = Student Error
task_durations_cond2 <- task_durations_cond2 %>%
  mutate(task_condition = if_else(task_condition == "Correct", "No Student Error", "Student Error"))
# Change values in gaze_condition so that On = Gaze On and Off = Gaze Off
task_durations_cond2 <- task_durations_cond2 %>%
  mutate(gaze_condition = if_else(gaze_condition == "On", "Gaze On", "Gaze Off"))
...
ggplot(task_durations_cond2,
  ...
  facet_wrap(
    ~ gaze_condition, 
    ncol   = 2                     # two panels side by side 🎨 [facet_wrap](https://www.google.com/search?q=ggplot2+facet_wrap)
  ) +
  labs(
    title = "", #Median Task Duration by Instructional Strategy\nand Task Condition, by Gaze Condition
    x     = "", #Student Error Condition
    y     = "Median Duration (s)",
...
ggplot(task_durations_type2,
  ...
  facet_wrap(
    ~ task_type,
    scales = "free_x",            # allow differing x‐axis labels length
    ncol   = 2
  ) +
  labs(
...
```
This figure shows that interestingly, when the student made an error that 
...
task_durations <- task_durations_cond2 %>%
  group_by(gaze_condition, instructional_strategy) %>%
  mutate(
...
# 1. Overall distribution by Strategy × Correctness × Gaze
...
donut_combined <- p_off + p_on +
  ...
  plot_annotation(title = "Task Duration Distribution by Gaze Condition",
                  subtitle = "Showing Strategy × Correctness proportions within each gaze condition",
                  theme = theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
print(sb_full)
cat("\n=== Full Sunburst: Gaze → Correctness → Strategy ===\n")

# Alternative: Create two separate sunbursts for each gaze condition
# This makes it easier to see the correctness → strategy breakdown
...
p_sunburst <- plot_ly(
  ...
  layout(
    title = "Task Duration Hierarchy: Gaze → Correctness → Strategy",
...
p_off_incorrect <- ggplot(
  ...
  aes(x = 1.0, y = total_count, fill = instructional_strategy)
) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  geom_text(aes(x = 1.2, label = label), position = position_stack(vjust = 0.5), size = 4) +
  xlim(0.5, 1.5) +
  labs(title = "Gaze Off - Student Error") +
  scale_fill_manual(values = strategy_palette) +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
...
p_off_correct <- ggplot(
  ...
  geom_text(aes(x = 1.2, label = label), position = position_stack(vjust = 0.5), size = 4) +
  xlim(0.5, 1.5) +
  labs(title = "Gaze Off - No Student Error") +
  scale_fill_manual(values = strategy_palette) +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
...
p_on_incorrect <- ggplot(
  ...
  geom_text(aes(x = 1.2, label = label), position = position_stack(vjust = 0.5), size = 4) +
  xlim(0.5, 1.5) +
  labs(title = "Gaze On - Student Error") +
  scale_fill_manual(values = strategy_palette) +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
...
p_on_correct <- ggplot(
  ...
  geom_text(aes(x = 1.2, label = label), position = position_stack(vjust = 0.5), size = 4) +
  xlim(0.5, 1.5) +
  labs(title = "Gaze On - No Student Error",
       fill = "Instructional Strategy") +
  scale_fill_manual(values = strategy_palette) +
  theme_void() +
  theme(
...
# ---- put this once, before building plotly_data ----
label_map <- c("Incorrect" = "Student Error",
               "Correct"   = "No Student Error")

# (optional: make it explicit which display label corresponds to which token)
all_correctness <- c("Incorrect", "Correct")  # your existing vector
all_correctness_labels <- setNames(label_map[all_correctness], all_correctness)
...
for (gaze in all_gaze) {
  for (correct in all_correctness) {
    plotly_data <- rbind(plotly_data, data.frame(
      labels  = all_correctness_labels[[correct]],          # <-- DISPLAY label
      parents = paste("Gaze", gaze),
      values  = correctness_value,
      ids     = paste("Gaze", gaze, correct, sep = "-")     # <-- keep ID using original token
    ))
  }
}
...
# ---------------------------
# Color mapping by node id
# ---------------------------

# Root & ring colors
node_colors <- c(
  "Total"    = "#FFFFFF",
  "Gaze Off" = "#FFCC80",  # light orange
  "Gaze On"  = "#90CAF9",  # light blue

  # correctness ring (pastel families; easy black text)
  "Gaze-Off-Incorrect" = "#F28B82",  # pastel red
  "Gaze-Off-Correct"   = "#FFEE58",  # pastel yellow
  "Gaze-On-Incorrect"  = "#D1C4E9",  # pastel purple
  "Gaze-On-Correct"    = "#80DEEA"   # pastel cyan
)
# 12 unique **pastel** leaves (avoid dark tones → Plotly won't flip to white text)
leaf_colors <- c(
  # Off × Incorrect → reds (light)
  "Gaze-Off-Incorrect-Descriptive" = "#FF8A9A",
  "Gaze-Off-Incorrect-Spatial"     = "#FEA5A5",
  "Gaze-Off-Incorrect-Minimal"     = "#FFAAE4",

  # Off × Correct → yellows (light)
  "Gaze-Off-Correct-Descriptive"   = "#FFDB7D",
  "Gaze-Off-Correct-Spatial"       = "#FFBC83",
  "Gaze-Off-Correct-Minimal"       = "#ECD7BE",

  # On × Incorrect → purples (light)
  "Gaze-On-Incorrect-Descriptive"  = "#E9D5FF",
  "Gaze-On-Incorrect-Spatial"      = "#E1BEE7",
  "Gaze-On-Incorrect-Minimal"      = "#D7BCE8",

  # On × Correct → teals (light)
  "Gaze-On-Correct-Descriptive"    = "#CFFAFE",
  "Gaze-On-Correct-Spatial"        = "#99F6E4",
  "Gaze-On-Correct-Minimal"        = "#A7F3D0"
)

# Merge rings + leaves
node_colors <- c(node_colors, leaf_colors)

# Assign colors to your rows by id (robust to order)
plotly_data$color <- unname(node_colors[plotly_data$ids])
...
p_sunburst_balanced <- plot_ly(
  ...
  textinfo = "label+percent parent",
  textfont = list(size = 24)                    # base text size for this trace
) %>%
  layout(
    title = list(text = "", font = list(size = 24)),
    margin = list(l = 50, r = 50, t = 100, b = 50),
    font = list(size = 24),                      # UI fonts (titles/legend)
    uniformtext = list(minsize = 16, mode = "show"),  # <- key: prevent tiny text
...
p_sunburst_simple <- ggplot(sb_simple$rects) +
  ...
  labs(
    title = "Simplified Sunburst: Gaze → Task → Strategy",
...
outliers_cond <- task_durations_cond %>%
  ...
  mutate(
    Q1    = quantile(mean_task_duration, 0.25),    # lower quartile {🔢} [quantile](https://www.google.com/search?q=R+quantile)
    Q3    = quantile(mean_task_duration, 0.75),    # upper quartile
    IQR   = Q3 - Q1,                               # interquartile range
    lower = Q1 - 1.5 * IQR,                        # whisker lower bound {📏} [IQR method](https://www.google.com/search?q=IQR+outlier+detection)
...
```{r my_big_plot, fig.width=12, fig.height=6, dpi=300}
# 0. Load libraries
library(dplyr)    # data manipulation 🧮 [dplyr group_by](https://www.google.com/search?q=dplyr+group_by)
library(ggplot2)  # plotting 📊 [ggplot2 boxplot](https://www.google.com/search?q=ggplot2+boxplot)

# 1. Compute per-subject median durations, grouped by strategy × condition × gaze
task_durations_cond <- combined_segments_per_round_all_dt_merged %>%
  group_by(
    subject_id,
    instructional_strategy,
    task_condition,
    gaze_condition      # include for faceting 🔍 [dplyr group_by multiple](https://www.google.com/search?q=dplyr+group_by+multiple)
  ) %>%
  summarise(
    mean_task_duration = median(task_duration, na.rm = TRUE),
    .groups = "drop"
...
# 2b. Alternatively, IQR‐based detection:
...
library(purrr)    # map_dfr    🧰  # needed for group_split/map_dfr
# refs: dplyr anti_join 🔗 https://www.google.com/search?q=dplyr+anti_join
#       ggplot facet_wrap 🔗 https://www.google.com/search?q=ggplot2+facet_wrap

# --- 1) Per-subject medians (strategy × task_condition × gaze) ---------------
task_durations_cond <- combined_segments_per_round_all_dt_merged %>%
  group_by(subject_id, instructional_strategy, task_condition, gaze_condition) %>%
  summarise(mean_task_duration = median(task_duration, na.rm = TRUE), .groups = "drop") %>%
  mutate(
...
# Sanity check to avoid empty data → facet error
stopifnot(nrow(task_durations_clean) > 0)

# --- 4) Plot -----------------------------------------------------------------
ggplot(task_durations_clean,
       aes(x = gaze_condition, y = mean_task_duration, fill = instructional_strategy)) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.6) +
  facet_wrap(~ task_condition, ncol = 2) +
  labs(
...
# ── Option A: SWAP positions → x = Off/On; facet = Incorrect/Correct
# (keep fill = instructional_strategy)
p_swapped <- ggplot(task_durations_clean,
                    aes(x = gaze_condition,
                        y = mean_task_duration,
                        fill = instructional_strategy)) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.6) +  # {➡️} [position_dodge](https://www.google.com/search?q=ggplot2+position_dodge)
  facet_wrap(~ task_condition, ncol = 2) +                              # {🎨} [facet_wrap](https://www.google.com/search?q=ggplot2+facet_wrap)
  labs(
...
# ── Option B: SWAP and show Incorrect/Correct side-by-side inside each Off/On group
...
```{r}
# Assuming you’ve already built `task_durations_type2` that includes gaze_condition
